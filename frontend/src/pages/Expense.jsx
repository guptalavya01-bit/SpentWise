...
const API_BASE = "https://spentwise-l0kz.onrender.com/api";
...
const ExpensePage = () => {
  ...
  // Search filter — filters by description (frontend only)
  ...
  const averageExpense = useMemo(
    () =>
      searchedTransactions.length
        ? Math.round(totalExpense / searchedTransactions.length)
        : 0,
    [searchedTransactions, totalExpense],
  );
    ...
    <div className={styles.container}>
      ...
      <div className={styles.cardsGrid}>
        ...
        <FinancialCard
          ...
          label="Transactions"
          value={searchedTransactions.length}
          additionalContent={
            <div className="mt-2 text-xs text-gray-500 flex items-center">
              <Calendar className="w-3 h-3 mr-1" />{" "}
              {filter === "all" && !searchQuery ? "All records" : "Filtered records"}
            </div>
          }
      ...
      {/* Search bar */}
      <div className="-mx-4 lg:-mx-0 md:-mx-5">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-3 bg-white border border-gray-200 rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-purple-300 focus:border-purple-500 text-sm text-gray-700 placeholder-gray-400 transition-all"
          />
          {searchQuery && (
            <button
      ...
      <div className={styles.transactionsContainer}>
        ...
        <div className={styles.transactionsList}>
          {searchedTransactions
            .slice(0, showAll ? searchedTransactions.length : 8)
            .map((transaction) => (
              <TransactionItem
                key={transaction.id}
          ...
          {!showAll && searchedTransactions.length > 8 && (
            <button
              onClick={() => setShowAll(true)}
              className={styles.viewAllButton}
            >
              <Eye size={18} /> View All {searchedTransactions.length}{" "}
              Transactions
            </button>
          )}
